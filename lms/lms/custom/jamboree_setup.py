...
def _log(message):
	frappe.logger("jamboree_setup").info(message)
...
def assign_role_profile_to_user(user_email, profile_name):
	"""Assign an LMS Role Profile to a user (for testing/setup)."""
	if not frappe.db.exists("User", user_email):
		_log(f"SKIP: User {user_email} does not exist")
		return
	if not frappe.db.exists("Role Profile", profile_name):
		_log(f"SKIP: Role Profile {profile_name} does not exist")
		return
	...
	_log(f"OK: {user_email} -> {profile_name} => {roles}")


def setup_test_users():
	"""Assign role profiles to test users for E2E testing."""
	assignments = {
		"amit.patel@example.com": "LMS HR",
		"priya.sharma@example.com": "LMS Master Trainer",
		"rahul.verma@example.com": "LMS Trainer",
		"sanjay.mehta@example.com": "LMS Manager",
		"dev.pandey@example.com": "LMS Student",
	}
...
def test_e2e():
	...
	for role_label, email in test_users.items():
		...
		try:
			...
			_log(f"[{role_label}] {email}: flags={json.dumps(flags)}, employee={emp_str}")
		except Exception as e:
			_log(f"[{role_label}] {email}: ERROR - {e}")
		finally:
	...
	try:
		...
		_log(f"get_hr_employees: {len(result['employees'])} employees, total={result['total_count']}")
		filters = get_hr_filters()
		_log(f"get_hr_filters: {len(filters['departments'])} depts, {len(filters['designations'])} desig, {len(filters['role_profiles'])} profiles")
		options = get_employee_options()
		_log(f"get_employee_options: {len(options)} employees")
	except Exception as e:
		_log(f"HR API ERROR: {e}")
	finally:
		frappe.set_user("Administrator")
	...
	try:
		...
		_log(f"get_manager_dashboard: {len(result['reports'])} reports, summary={result['summary']}")
		for r in result['reports']:
			enrollments = r.get('enrollments', [])
			_log(f"  {r['employee_name']}: {len(enrollments)} enrollments, avg={r['avg_progress']}%")
			for e in enrollments:
				_log(f"    - {e.get('course_title')}: {e['progress']}%")
	except Exception as e:
		_log(f"Manager API ERROR: {e}")
	finally:
		frappe.set_user("Administrator")
	...
	try:
		...
		_log(f"get_trainer_dashboard: {len(result['batches'])} batches, summary={result['summary']}")
		for b in result['batches']:
			for s in b.get('students', []):
				enrollments = s.get('enrollments', [])
				_log(f"  Student {s['member_name']}: {len(enrollments)} enrollments")
				for e in enrollments:
					_log(f"    - {e.get('course_title')}: {e['progress']}% ({e.get('status')})")
	except Exception as e:
		_log(f"Trainer API ERROR: {e}")
	finally:
		frappe.set_user("Administrator")
	...
	try:
		...
		get_hr_employees()
		_log("FAIL: Student should NOT access get_hr_employees!")
	except frappe.PermissionError:
		_log("PASS: Student blocked from get_hr_employees")
	except Exception as e:
		_log(f"PASS (unexpected): {e}")
	finally:
		frappe.set_user("Administrator")
	...
	try:
		...
		_log("FAIL: Student should NOT access get_manager_dashboard!")
	except frappe.PermissionError:
		_log("PASS: Student blocked from get_manager_dashboard")
	except Exception as e:
		_log(f"PASS (unexpected): {e}")
	finally:
		frappe.set_user("Administrator")
	...
	try:
		...
		_log("FAIL: Student should NOT access get_trainer_dashboard!")
	except frappe.PermissionError:
		_log("PASS: Student blocked from get_trainer_dashboard")
	except Exception as e:
		_log(f"PASS (unexpected): {e}")
	finally:
		frappe.set_user("Administrator")
	...
	try:
		...
		if courses:
			...
			if already:
				_log(f"Student already enrolled in {course}, skipping assignment test")
			else:
				result = assign_course_to_student("dev.pandey@example.com", course)
				_log(f"assign_course_to_student: {result}")
		recent = get_recent_assignments()
		_log(f"get_recent_assignments: {len(recent)} recent")
	except Exception as e:
		import traceback
		_log(f"Course Assignment ERROR: {e}")
		traceback.print_exc()
	finally:
		frappe.set_user("Administrator")
	...
	try:
		...
		_log(f"get_student_assignments (own): {len(result)} enrollments")
	except Exception as e:
		_log(f"Student Assignment ERROR: {e}")
	finally:
		frappe.set_user("Administrator")
...
def create_jamboree_role_profiles():
	...
	profiles = {
		"LMS Student": ["Employee", "LMS Student"],
		"LMS Trainer": ["Employee", "LMS Student", "LMS Trainer"],
		"LMS Master Trainer": [
			"Employee",
			"LMS Student",
			"LMS Trainer",
		...
		"LMS Manager": ["Employee", "LMS Student", "LMS Manager"],
		"LMS HR": [
			"Employee",
			"LMS Student",
			"LMS Trainer",
	...
	for profile_name, roles_list in profiles.items():
		# The "Employee" role ships with HRMS/ERPNext and may be absent on a
		# pure-LMS bench; skip any role that does not exist on this site.
		roles_list = [role for role in roles_list if frappe.db.exists("Role", role)]
