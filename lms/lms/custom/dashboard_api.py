...
from lms.lms.custom import employee_doctype_exists, require_employee_doctype


@frappe.whitelist()
def get_direct_reports(manager_employee_id: str | None = None):
    """Get direct reports for a manager using Employee.reports_to."""
    require_employee_doctype()
...
def get_hr_filters():
    ...
    # Department and Designation ship with HRMS/ERPNext and may be absent
    # on a pure-LMS bench.
    departments = (
        frappe.get_all("Department", pluck="name", order_by="name asc")
        if frappe.db.exists("DocType", "Department")
        else []
    )
    designations = (
        frappe.get_all("Designation", pluck="name", order_by="name asc")
        if frappe.db.exists("DocType", "Designation")
        else []
    )
    role_profiles = frappe.get_all(
        "Role Profile",
        filters={"name": ["like", "LMS%"]},
        pluck="name",
        order_by="name asc",
    )
...
@frappe.whitelist()
def save_employee_lms_role(employee: str, role_profile: str):
    """HR: Assign an LMS role profile to an employee."""
    require_employee_doctype()
    frappe.only_for("LMS HR")
...
@frappe.whitelist()
def export_team_progress(manager_employee_id: str | None = None):
...
@frappe.whitelist()
def update_employee_manager(employee: str, reports_to: str | None):
...
@frappe.whitelist()
def unassign_employee_course(enrollment: str):
...
@frappe.whitelist()
def unassign_employee_role(employee: str):
...
def deactivate_employee(
    employee: str, status: str = "Inactive", relieving_date: str | None = None
):
...
@frappe.whitelist()
def reactivate_employee(employee: str):
...
@frappe.whitelist()
def get_locked_chapters_for_employee(employee: str, batch: str):
...
@frappe.whitelist()
def unlock_chapter_for_employee(employee: str, course: str, chapter: str):
