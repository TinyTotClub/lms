...
def check_student_progress_alerts():
	...
	# Manager alerts are resolved via the Employee hierarchy, which requires
	# HRMS/ERPNext. Silently skip on a pure-LMS bench.
...
def notify_on_course_completion(doc, method):
	...
	# Notify manager via Employee hierarchy (requires HRMS/ERPNext; silently
	# skip the manager notification on a pure-LMS bench).
