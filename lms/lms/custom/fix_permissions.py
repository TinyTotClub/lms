...
def _log(message):
	frappe.logger("fix_permissions").info(message)
...
def delete_all_lms_custom_docperms():
	...
	for doctype in lms_doctypes:
		...
		_log(f"Deleted all Custom DocPerms for {doctype}")
...
def recreate_all_lms_permissions():
	...
	for perm in all_permissions:
		...
		_log(f"Created: {perm['parent']} - {perm['role']}")
