...
from lms.lms.custom import employee_doctype_exists
...
def add_batch_enrollment_approval_fields():
	...
	# The employee fields link to the Employee doctype, which ships with
	# HRMS/ERPNext and may be absent on a pure-LMS bench. Skip just these
	# fields when it is missing; the rest of the approval fields still apply.
	if employee_doctype_exists():
		fields.extend(
			[
				{
					"fieldname": "employee",
	...
	custom_fields = {"LMS Batch Enrollment": fields}
...
def get_manager_permission_query_condition(user, doctype):
	...
	try:
		...
		# Without the Employee doctype (no HRMS) there is no manager hierarchy,
		# so fall back to the restrictive default: show only own enrollments.
...
@frappe.whitelist()
def approve_enrollment(name: str, comments: str | None = None):
	"""
...
@frappe.whitelist()
def reject_enrollment(name: str, comments: str | None = None):
	"""
...
def can_manage_enrollment(doc):
	...
	# Without the Employee doctype (no HRMS) there is no manager hierarchy,
	# so nobody besides the roles above can manage enrollments.
	if not employee_doctype_exists():
		return False
...
def set_employee_field(doc, method=None):
	...
	# Get employee linked to this user (Employee ships with HRMS/ERPNext and
	# may be absent on a pure-LMS bench).
	if employee_doctype_exists():
